...
def watercooler_break(interval):
    ...
    if end_P1 < price_df.iloc[ix]['index2'] < end_P2:
        return True
...
@app.callback([Output("interval-component", "disabled"), Output("submit", "disabled"),\
               ...
               [Input("interval-component", "n_intervals"), Input("bid_submitted1",'children'), \
                Input("bid_submitted2",'children'), Input('user-begin', 'n_clicks'), \
                Input('data-end', 'data'), Input("watercooler","data")],
...
def toggle_interval_for_bid(interval, bid_submitted1, bid_submitted2, begin_click, dataend, wc, cash, price, stock):
    ...
    if ix>MAX_LEN:
        ...
        return False, True, 'Trading paused due to high volume. Feel free to take a 5 minute break.'
    elif begin_click:
...
def end_experiment(exp_end, x1, x2, p1, p2, mturk):
    ...
    if not mturk:
        raise PreventUpdate
    conn = sqlite3.connect('database_app2.db')
    sql = '''INSERT INTO results VALUES(?,?,?,?,?,?)'''
    conn.execute(sql, ("app2", mturk, x1, p1, x2, p2))
    conn.commit()
    conn.close()
