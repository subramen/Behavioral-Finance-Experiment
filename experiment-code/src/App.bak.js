...
const configState =  selector({
  ...
  get: ({get}) => {
    ...
    return {
      ...
      T2: 60 * intervalMultiplier,
      TN: 100 * intervalMultiplier,
      WC: false,
      price0: 0,
      initCash: 1000,
    };
...
const isTimeToTrade = selector({
  ...
  get: ({get}) => {
    ...
    return ((T1 <= nowIdx && nowIdx < T1 + 30*intervalMultiplier) || (T2 <= nowIdx && nowIdx < T2 + 30*intervalMultiplier));
  }
});
...
const Intro = ({setAMZN}) => {
  ...
  // const setAMZN = useSetRecoilState(amznID);
...
const Experiment = () => {
  ...
  const display = (amznID !== '') ? <ExperimentInterface /> : <Intro setAMZN={setAMZN}/>
...
const StateManager = ({expPause}) => {
  ...
  useEffect(() => {
      ...
      const interval = setInterval(() => {
          if (nowIdx < TN && !expPause) setNowIdx(nowIdx + intervalMultiplier);            
    ...
    { 
      ...
      if (isTradeTime) {
        elt = ( <div className='status-msg'>
          ...
          <Countdown id='count' then = {Date.now() + 350000} />
        </div> );
      }
      else {
      elt = ( <div className='status-msg'>
  ...
  const TradingPanel = () => {
    ...
    return (
      <div style={{alignSelf: 'center'}}>
        {maxQtyStatus}
        <fieldset className="trading-panel">
