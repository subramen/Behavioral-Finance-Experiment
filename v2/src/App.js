...
class App extends React.Component {
  ...
  render() {
    ...
    return (
      <div className="App">
        <div className="container" style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gridGap: 20 }}>
          <PriceTracker price={price} timestamp={timestamp} />
          <TradeCenter active={this.state.tradeActive} price={price}/>
        </div>
      </div>
    );
  }
...
class TradeCenter extends React.Component{  
  ...
  render() {
    ...
    return (
      <div style={{display:"grid", gridTemplateRows: "1fr 1fr 2fr 2fr"}}>
        <div/>
        <Message active={this.props.tradeActive} />
        <CtrlPanel price={this.props.price} cash={cash} stocks={stocks} processTrade={this.processTrade}/>    
...
function Message (props) {
  const out = (props.tradeActive ? "Enter a trade" : "Observe the market");
  return (
    <h2>{out}</h2>
  );
}
...
function StatusTable(props) {
  ...
  <table className="greenTable">
    <tbody>
      ...
      <tr>
        <th>Portfolio</th>
        <td>${Math.round((stocks * price)*100)/100}</td>
      </tr>
      <tr>
        <th>Stocks Held</th>
        <td>{stocks}</td>
...
class CtrlPanel extends React.Component {
  ...
  render() {
    return (
      <div>
        <fieldset >
          <legend align='left'>Buy/Sell Panel</legend>
          
          <div className='inputBox'>
            <label htmlFor='in'>No. of Stocks: </label>
            <input className='in' type='number' placeholder='0' min='0' onChange={this.handleInputChange}></input>
          </div>
          <br/>
          ...
          <input className='sbmt' type="submit" value="Submit" onClick={this.handleSubmit}/>
      </fieldset>
      </div>
...
class TitleBar extends React.Component {
  ...
  getTicker() {
    ...
    var priceDiff = Math.round((this.props.price - this.prevPrice + Number.EPSILON) * 100) / 100;

    out = (currPrice < this.prevPrice ? 
      <Ticker color="red" cp={currPrice} pd={priceDiff}/> :
      <Ticker color="green" cp={currPrice} pd={priceDiff}/>); 
...
function Ticker(props) {
  const {color, cp, pd} = props;
  const triangle = (color === "red" ? "dn-triangle" : "up-triangle")
  return (
    <div className="Ticker" style={{color:color}}>
      <h2 style={{justifySelf: "end"}}>{cp}</h2>
      <span id={triangle}/> 
      <h2 style={{justifySelf: "start"}}>{pd}</h2>
    </div>
  );
}
